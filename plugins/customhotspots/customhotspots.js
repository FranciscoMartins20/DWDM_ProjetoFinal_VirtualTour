$(document).ready(function () {
    // $(".editortab").eq(1).after('<div class="editortab" onclick="chShowHotspotIcons()"><i class="fa fa-arrow-circle-up" style="width: 20px;"></i> Icons Hotspot</div>');
});
...
function chShowHotspotIcons() {
    ...
    // $("#editorcontent").html("<h2>Icons Hotspot</h2>" + imagefiles + "<div class='imgthumb' onclick='chAddImageasset()'><div style='cursor: pointer; display: table; width: 100%; height: 100%;'><div style='display: table-cell; text-align: center; vertical-align: middle;'><i class='fa fa-plus' style='font-size: 40px;'></i></div></div></div>");
}
...
function showIconChooser(cid, title) {
    ...
    $("#dimmessage").html("").html("<div style='width: 100%; max-width: 720px; height: 100%; margin: 0 auto;'><div style='background-color: #2c3643; color: white; padding: 10px;'><i class='fa fa-question-circle'></i> " + title + "</div><div style='padding: 30px; background-color: #3d4855; font-size: 14px; font-weight: normal;'><div style='box-sizing: border-box; width: 100%; height: " + (
        innerHeight - 400
    ) + "px; overflow: auto;'><p>Default Icons:</p><div>" + itemstochoose + "</div><p>Custom Icons:</p><div>" + customhotspots + "</div></div><button onclick='hideDim()' style='margin-left: 10px; margin-top: 20px; margin-bottom: 0px;'><i class='fa fa-times'></i> Close</button></div></div>");
    $("#dim").show();
    $("#loading").hide();
    ...
    for (var i = 0; i < currentprojectdata.panoramas.length; i++) {
        ...
        if (currentprojectdata.panoramas[i].hotspots.length == 0) {
            ...
            for (var x = 0; x < currentprojectdata.panoramas[i].hotspots.length; x++) {
                ...
                var currenthActions = "<p>" + i18next.t('no-hotspot-action') + "</p>";
                ...
                if (hactions.length > 0) {
                    ...
                    for (var y = 0; y < hactions.length; y++) {
                        var hatype;
                        var htarget = hactions[y].target.split("/")[1].substring(0, 26);
                        if (hactions[y].target.split("/")[1].length > 26) {
                            htarget += "...";
                        }
